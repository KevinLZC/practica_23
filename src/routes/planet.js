...
router.get('/findById/:id', (req, res) => { // nos permite encontrar el objeto para obtener sus propiedades
  Planet.findById(req.params.id)
  .then((myPlanet) => {res.render('planetUpdate', {myPlanet})})
  .catch((error) => {res.json({message:error})});
});
router.post('/updatePlanet', (req, res) => { // ruta para mandar la información a la base de datos
  let rings = false;
  if(req.body.rings === 'on') {
    rings = true;
  }
...
router.get('/deletePlanet/:id', (req, res) => { // nos permite encontrar el objeto para obtener sus propiedades
  Planet.findByIdAndDelete(req.params.id)
  .then((data) => {res.redirect('/planets')})
  .catch((error) => {res.json({message:error})}); // nos regresa un error
});
